...
@router.post("/plan")
async def plan_session(payload: SessionPlanRequest) -> dict[str, object]:
    if not supabase:
    ...
    plan = await asyncio.to_thread(generate_session_plan, payload, context_chunks)
    ...
    inserted = await asyncio.to_thread(lambda: supabase.table("sessions").insert(insert_payload).execute())
...
async def update_task(session_id: str, task_index: int, payload: TaskUpdateRequest) -> dict:
    ...
    existing = await asyncio.to_thread(
        lambda: supabase.table("sessions").select("id,tasks").eq("id", session_id).limit(1).execute()
    )
    ...
    updated = await asyncio.to_thread(
        lambda: supabase.table("sessions").update({"tasks": tasks}).eq("id", session_id).execute()
    )
